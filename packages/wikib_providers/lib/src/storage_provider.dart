...
Provider<StorageInfo> getStorageInfoProvider<TStorage extends Storage>(
        AlwaysAliveProviderBase<DBRewiseId?> dbIdProvider, Provider<TableAccount?> tableAccountProvider) =>
...
FutureProvider<TStorage?> getStorageProvider<TStorage extends Storage>(
  ...
  StateProvider<TStorage?> oldStorageProvider,
    ...
    FutureProvider<TStorage?>((ref) async {
      ...
      if (oldStorage.state != null &&
        ...
        assert(storageInfo.hiveBox != oldStorage.state!.info.hiveBox);
        await oldStorage.state!.moveTo(res);
      } else {
        if (oldStorage.state != null && oldStorage.state!.info.hiveBox != storageInfo.hiveBox) await oldStorage.state!.close();
        assert(storageInfo.hiveBox.isOpen);
        await res.initialize();
      }
      oldStorage.state = res;
      return res;
    }, name: 'StorageProviders.${TStorage.runtimeType}');

StateProvider<TStorage?> getOldStorageProvider<TStorage extends Storage>() =>
    StateProvider<TStorage?>((_) => null, name: 'oldStorageProvider.${TStorage.runtimeType}');

Provider<Future Function()?> getDebugStorageDeleteProvider<TStorage extends Storage>(
  Provider<StorageInfo> storageInfoProvider,
) =>
    Provider<Future Function()?>((ref) {
      final storageInfo = ref.watch(storageInfoProvider);
      return storageInfo.isEmpty
          ? null
          : () async {
              final hive = await Hive.openBox(storageInfo.hiveName);
              await hive.deleteFromDisk();
              if (storageInfo.tableAccount != null) await Storage.debugDeleteAzureAll(storageInfo.partitionKey, storageInfo.tableAccount!);
            };
